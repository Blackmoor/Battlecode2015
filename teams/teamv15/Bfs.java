...
public class Bfs {
	...
	private static void writePageMetadata(int page, int roundLastUpdated, MapLocation dest, int priority, boolean finished) throws GameActionException {
		...
		int data = (finished ? 1<<29 : 0) | ((priority & 0x3) << 27) | (roundLastUpdated << 16) | (cropX(dest.x) << 8) | cropY(dest.y);
...
//	private static Direction[][] path = null; //Filled in when we determine a direction - the master copy is broadcast, this is just for debug
	...
	private static void initQueue(MapLocation dest) {
		...
		if (isBlocked == null) {
			...
			if (hq.x == ehq.x)
				symmentry = 1; // reflection on y axis
			else if (hq.y == ehq.y)
				symmentry = 2; // reflection on x axis
				...
				for (int x=minX; x < maxX; x++) {
					...
					if (tt == TerrainTile.UNKNOWN) {
						...
						case 0: //rotation
							...
							break; 
						case 1: // reflection on y axis
							tt = rc.senseTerrainTile(new MapLocation(x, hq.y + ehq.y - y));
							break;
						case 2: // reflection on x axis
		...
		//Don't go in the shooting range of the enemy HQ (unless the dest in range)
		MapLocation enemyHQ = rc.senseEnemyHQLocation();
		if (dest.distanceSquaredTo(enemyHQ) > RobotType.HQ.attackRadiusSquared) {
			MapLocation[] danger = MapLocation.getAllMapLocationsWithinRadiusSq(enemyHQ,RobotType.HQ.attackRadiusSquared);
			for (MapLocation m:danger)
				processed[cropX(m.x)][cropY(m.y)] = true; 	
		}
...
//				path[x][y] = Direction.NONE;
	...
	public static void work(MapLocation dest, int priority, int stopWhen) throws GameActionException {
	...
	private static void publishResult(int page, MapLocation here, MapLocation dest, Direction dir, int actions) throws GameActionException {
		int data = 0x80;
		data |= dir.ordinal();
		data <<= 8;
		data |= actions;

...
public class RobotPlayer {
	...
	public static void run(RobotController tomatojuice) {
		...
		try {
			spawnID = rc.readBroadcast(65000);
			rc.broadcast(65000, spawnID+1);
		...
		while(true) {	
			...
			if (rc.isCoreReady()) {
				try {
					if (rc.getType().canSpawn()) {
						...
						if (build != null)
							trySpawn(rc.getLocation().directionTo(rc.senseEnemyHQLocation()), build);							
					} else if (rc.getType().canBuild()) {
						RobotType build = getBuildOrder();
						if (build != null && rc.hasBuildRequirements(build))
							tryBuild(rc.getLocation().directionTo(rc.senseEnemyHQLocation()), build);
					}
				} catch (GameActionException e) {
			...
			//Move if possible
			//Combat units should move towards enemy units and away from our other units
					...
					if (nearest != null) {
						...
						if (spawnID % 2 == 0 || rc.getType() == RobotType.TANK) {
							...
							if (rc.getType() != RobotType.DRONE) // Drones can fly over voids
								lookup = Bfs.readResult(rc.getLocation(), ehq);
							if (lookup == null)
								lookup = rc.getLocation().directionTo(ehq);
							tryMove(lookup);
			...
			if (rc.getType() == RobotType.HQ && Clock.getRoundNum() > 600 && Clock.getBytecodesLeft() > 1000) {
				try {
					Bfs.work(rc.senseEnemyHQLocation(), Bfs.PRIORITY_HIGH, 1000);
				} catch (GameActionException e) {
	...
	private static void tryMove(Direction preferred) throws GameActionException {
		...
		int dirint = directionToInt(preferred);
		...
		if (!overrideThreat) { // We override the threat level if the clock is past 1700 or we have twice the health of his units
			...
			if (ehq.distanceSquaredTo(myLoc) <= senseRange)
		...
		for (int offsetIndex=0; offsetIndex < offsets.length; offsetIndex++) { //Loop over the possible moves and pick the first good one
			Direction d = directions[(dirint+offsets[offsetIndex]+8)%8];
			...
			if (rc.canMove(d)) {
				...
				if (ehq.distanceSquaredTo(tile) <= RobotType.HQ.attackRadiusSquared)
					threatened = true;
				...
				if (!threatened)
					for (RobotInfo u: units) {
						if (u.team == enemyTeam && u.type.canAttack() && u.location.distanceSquaredTo(tile) <= u.type.attackRadiusSquared) {
							threatened = true;
							break;
						}
					}
